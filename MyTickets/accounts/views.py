    ...
    if request.method == 'POST':
        ...
        if len(trains)==0:
            ...
            for train in trains:
                ...
                train_seat_data = Train.objects.filter(train_number = train.train_number)
                reservation = Reservation.objects.filter(train__train_number=train.train_number)
                train_details["Available Seat Count"] = train_seat_data[0].numberOfSeats - len(reservation)
