...
class TrainAvailableSerializer(serializers.ModelSerializer):
    ...
    def numOfSeats(self, obj):
        ...
        reservation = Reservation.objects.filter(train__train_number=request.data["train_number"])
        train_data = Train.objects.filter(train_number = request.data["train_number"])
        return (train_data[0].numberOfSeats - len(reservation))
